// Script to reset the database and populate it with fresh test data
...
async function resetDatabase() {
  ...
  try {
    ...
    await db.update(departments)
      ...
      .where(eq(departments.id, computerScience[0].id));
    
    await db.update(departments)
      .set({ headId: baHod[0].id })
      .where(eq(departments.id, business[0].id));
    ...
    // Create levels - now general across departments
